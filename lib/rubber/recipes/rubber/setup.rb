...
namespace :rubber do
  ...
  task :install_core_packages do
    ...
    install_commands = core_packages.collect do |package|
      "apt-get -q -o Dpkg::Options::=--force-confold -o APT::Get::allow-downgrades=true -o APT::Get::allow-change-held-packages=true -y install #{package} || true"
    end
    ...
    if upgrade
      if ENV['NO_DIST_UPGRADE']
        sudo_script 'upgrade_packages', <<-ENDSCRIPT
          ...
          apt-get -q -o Dpkg::Options::=--force-confold -o APT::Get::allow-downgrades=true -o APT::Get::allow-change-held-packages=true -y upgrade
        ENDSCRIPT
      else
        sudo_script 'dist_upgrade_packages', <<-ENDSCRIPT
          ...
          apt-get -q -o Dpkg::Options::=--force-confold -o APT::Get::allow-downgrades=true -o APT::Get::allow-change-held-packages=true -y dist-upgrade
        ENDSCRIPT
      end
    else
      install_packages_script = <<-ENDSCRIPT
        ...
        apt-get -q -o Dpkg::Options::=--force-confold -o APT::Get::allow-downgrades=true -o APT::Get::allow-change-held-packages=true -y install $@
      ENDSCRIPT
