      ...
      def setup_security_groups(host=nil, roles=[])
        ...
        script = <<-ENDSCRIPT
          ...
          if ! dpkg -l iptables &> /dev/null; then
            export DEBIAN_FRONTEND=noninteractive; apt-get -q -o Dpkg::Options::=--force-confold -o APT::Get::allow-downgrades=true -o APT::Get::allow-change-held-packages=true -y install iptables
          fi
